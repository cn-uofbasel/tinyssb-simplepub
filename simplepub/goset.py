...
class GOset():
    ...
    def __init__(self, node, keys=[], verbose=False) -> None:
        self.node = node
    ...
    def incoming_goset_msg(self, dmx, pkt: bytes) -> list:
        ...
        if cl.sz == len(self.keys) and self.state == cl.xo:
            ...
            syn = "/not in sync"
        if self.verbose:
            print(f"   =G claim span={cl.sz} lo={i1}:{cl.lo[:10].hex()}.. hi={i2}:{cl.hi[:10].hex()}...  {syn}")
        ...
        if i2 - i1 == 1 and cl.sz == 3: # we just lack the key in the middle
            ...
            for i in range(len(xor)):
                ...
                xor[i] ^= cl.hi[i]
            if self.verbose:
                print(f"   computed the missing middle key {xor.hex()}")
    ...
    def get_adv(self) -> (list, int):
        ...
        if cl.xo != self.state:
            if self.verbose:
                print("   Gadv - new GOset state", cl.xo.hex(), "|keys|=", len(self.keys))
            self.state = cl.xo
            self.node.set_want_dmx(self.state)
        ...
        for c in self.pending_claims:
            ...
            if partial.sz <= c.sz:
                if max_ask > 0:
                    max_ask -= 1
                    if self.verbose:
                        print(f"   Gadv need {lo}..{hi} @{len(self.keys)}")
            ...
            if max_help > 0:
                ...
                if hi <= lo:
                    ...
                    if self.verbose:
                        print(f"   Gadv have {lo}..{hi} @{len(self.keys)}")
                    lst.append(self.goset_dmx + self.mkClaim(lo,hi).wire)
                else: # split span in two intervals
                    sz = (hi + 1 -lo) // 2
                    if self.verbose:
                        print(f"   Gadv have {lo}..{lo+sz-1} @{len(self.keys)}")
                        print(f"   Gadv have {lo+sz}..{hi} @{len(self.keys)}")
